...
function FeatureClient(config) {
  ...
  if (config.shared) {
    this.sharedDevKey = config.shared.devKey || process.env.FEATURE_DEVKEY_SHARED;
    delete config.shared.devKey;

    // Shared experiments can be empty if you're just reading.
    // Use-case: this is the server and the shared is the client.
    if (config.shared.experiments) config.shared.experiments = config.shared.experiments.map(cleanupExperiment);
  }
...
FeatureClient.prototype.announce = function(cb) {
  ...
  if (this.sharedDevKey) request.set('x-feature-key-shared', this.sharedDevKey);
